...
pub fn fs_read_file(path: String, workspace: Option<WorkspaceEnv>) -> Result<ReadResult, String> {
    ...
    if let Some(conn) = workspace.ssh_conn() {
        return crate::modules::ssh::read_file(conn, &path);
    }
...
pub fn fs_write_file(
    ...
    let workspace = WorkspaceEnv::from_option(workspace);
    if let Some(conn) = workspace.ssh_conn() {
        crate::modules::ssh::write_file(conn, &path, &content)?;
        let _ = app.emit(
            "fs:file-written",
            FileWrittenEvent {
                path: path.clone(),
...
pub fn fs_canonicalize(path: String, workspace: Option<WorkspaceEnv>) -> Result<String, String> {
    let workspace = WorkspaceEnv::from_option(workspace);
    if let Some(conn) = workspace.ssh_conn() {
        return crate::modules::ssh::canonicalize(conn, &path);
    }
    let p = resolve_path(&path, &workspace);
...
pub fn fs_stat(path: String, workspace: Option<WorkspaceEnv>) -> Result<FileStat, String> {
    let workspace = WorkspaceEnv::from_option(workspace);
    if let Some(conn) = workspace.ssh_conn() {
        return crate::modules::ssh::stat(conn, &path);
    }
    let p = resolve_path(&path, &workspace);
