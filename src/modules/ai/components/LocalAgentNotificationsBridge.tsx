...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "Skiff needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "Skiff run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "Skiff finished", "Your task is ready");
    }
  }, [status, error]);
