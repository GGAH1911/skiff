...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    ...
    case "fedora":
      return `sudo dnf install ./Skiff-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Skiff v${manual.version} is available`
                  : `Skiff v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Skiff to finish installing."
              : downloading
                ? progress !== null
